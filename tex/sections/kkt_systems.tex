...
\add{We suppose that the dual regularization is $\delta_c = 0$:
a positive dual regularization simplifies the solution of the
KKT system~\eqref{eq:kkt:condensed}, as explained in Remark~\ref{rmk:dualreg}.}
Then, the trick is to reformulate the condensed KKT system \eqref{eq:kkt:condensed} in an equivalent form
\begin{equation}
  \label{eq:kkt:hykkt}
  \begin{bmatrix}
    K_\gamma & G^\top \\
...
if and only if the condensed matrix $K_\gamma$ is positive definite.
...
\begin{remark}
  \label{rmk:dualreg}
  \add{
    The dual regularization $\delta_c$ can become positive during the IPM iterations.
    This is the case if the primal-dual regularization detects
    a degenerate Jacobian $G$ or if the filter line-search algorithm is entering a feasibility
    restoration phase~\cite[Section 3.3]{wachter2006implementation}.
    For a positive $\delta_c$ the Golub \& Greif strategy becomes not necessary, as
    solving the system \eqref{eq:kkt:condensed} translates to solving
    \begin{equation*}
      \left( K + \frac{1}{\delta_c} G^\top G \right) d_x = \bar{r}_1 - \frac{1}{\delta_c} \bar{r}_2 \;,
    \end{equation*}
    and setting $d_y = \delta_c^{-1} (G d_x - \bar{r}_2)$.
  }
\end{remark}

\begin{remark}
  \add{
    We have seen that the parameter $\gamma$ should be set above a given threshold $\underline{\gamma}$
    for $K_\gamma$ to be positive definite. However, a large value of $\gamma$ increase
    the ill-conditioning in $K_\gamma$, hence deteriorating the solution's quality.
    In \cite{golub2003solving}, the authors recommend choosing $\gamma = \| K \| / \|G\|^2$,
    % SS: we need to clarify what type of norm this is
    which is offering a good empirical trade-off.
    In \cite[Lemma 2.10]{gould2010spectral}, a tighter estimate is given for the threshold
    $\underline{\gamma}$, if $G$ has full row-rank and $Z^\top K Z$ is positive definite:
    \begin{equation}
    ...
    with $\sigma_{\min}$ the smallest nonzero singular value of the Jacobian $G$,
    $\lambda_{\min}^Z > 0$ the smallest eigenvalue of $Z^\top K Z$
    and $\lambda_{\min}^Y$ the smallest eigenvalue of $Y^\top K Y$.
    We observe from \eqref{eq:kkt:boundgamma} that $\underline{\gamma}$ is
    inversely proportional to $\lambda_{\min}^Z$ and $\sigma_{\min}$, meaning
    it can reach a large magnitude if $G$ has nearly aligned rows or
    $Z^\top K Z$ is closed to be indefinite.
  }
\end{remark}
...
whereas LiftedKKT factorizes a positive definite matrix $K_\tau$
