...
\end{equation}
Using \eqref{eq:cond:perturbationbound} and $(\Delta_L, \Delta_S)= O( \Xi^{-1}\epstol)$,
...
using HyKKT,
and show that the error remains reasonable even for large values of $\gamma$.
...
Y \widehat{s}_Y + Z \widehat{s}_Z$. Using \eqref{eq:cond:boundderivationhykkt},
...
The second writes as
\begin{equation}
  K_\gamma^{-1}\Delta K_\gamma (K_\gamma^{-1} \Delta s_\gamma)  =
  \big[ G U_L \Sigma_L^{-1} \Delta_L + G U_S \Sigma_S^{-1} \Delta_S \big] (K_\gamma^{-1} \Delta s_\gamma) \; .
\end{equation}
Using \eqref{eq:cond:invariantsubpsace} again, we have
$G U_L = G Y + O(\Xi)$ and $G U_S = GZ + O(\Xi) = O(\Xi)$. Hence
$G U_L \Sigma_L^{-1} \Delta_L = O(1)$ and $G U_S \Sigma_S^{-1} \Delta_S = O(1)$.
...
Similar as in the right-hand-side, the Schur-complement
...
\begin{proof}
  ...
  \begin{equation}
    \begin{aligned}
      \widehat{S}_\gamma &= \widehat{G} \widehat{K}_\gamma^{-1} \widehat{G}^\top \; , \\
                         &= (G + \Delta G)\big(K_\gamma^{-1} - K_\gamma^{-1}\Delta K_\gamma K_\gamma^{-1}\big)(G + \Delta G)^\top \;, \\
  ...
  Using \eqref{eq:cond:rhserror}, we get $K_\gamma^{-1} \Delta G^\top = O(\epstol)$
...
G U_L \Sigma_L^{-1} \Delta_L \Sigma_L^{-1}U_L^\top G^\top  +
G U_S \Sigma_S^{-1} \Delta_S \Sigma_S^{-1}U_S^\top  G^\top \;.
  \end{equation*}
  Using \eqref{eq:cond:invariantsubpsace}, we have $G U_S = O(\Xi)$,
  and as $\Sigma_S^{-1} = \Theta(1)$ and $\Delta_S = O(\Xi^{-1} \epstol)$, we
  get
  $G U_S \Sigma_S^{-1} \Delta_S \Sigma_S^{-1}U_S^\top  G^\top = O(\Xi \epstol)$.
  Finally, as $\Sigma_L^{-1} = \Theta(\Xi)$ and $G U_L = GY + O(\Xi)$,
  we have
  \begin{equation}
    G U_L \Sigma_L^{-1} \Delta_L \Sigma_L^{-1}U_L^\top G^\top =
    G Y \Sigma_L^{-1} \Delta_L \Sigma_L^{-1}Y^\top G^\top + O(\Xi^2 \epstol) \; .
  \end{equation}
  We conclude the proof by observing that
  $G Y \Sigma_L^{-1} \Delta_L \Sigma_L^{-1}Y^\top G^\top = O(\Xi \epstol)$.
\end{proof}
...
have only a few inequality constraints.
