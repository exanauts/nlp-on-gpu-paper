...
both the equality-relaxation strategy or the HyKKT strategy.
We will see that the we can tighten the bounds in \eqref{eq:cond:defaultbond}
...
This is equivalent to include the the bounds on the variable $x$ in the inequality constraints,
...
of $\Xi$ than the error $\Delta x_S$.
...
U_L \Sigma_L^{-1} \Delta_L \Sigma_L^{-1}U_L^\top  +
  U_S \Sigma_S^{-1} \Delta_S \Sigma_S^{-1}U_S^\top  \;.
\end{equation}
Using \eqref{eq:cond:perturbationbound} and $\Delta_L, \Delta_S = O(\epstol \Xi^{-1})$,
we deduce that the error made in the large-space is $O(\epstol \Xi)$ whereas
the error in the small-space is $O(\epstol \Xi^{-1})$.
...
Similarly, the diagonal $\widehat{D}_s$ remains full accuracy only
w.r.t. the inactive components: $\widehat{D}_{s,\cactive} = D_{s,\cactive} + O(\Xi \epstol)$
and $\widehat{D}_{s,\cinactive} = D_{s,\cinactive} + O(\Xi^{-1} \epstol)$.
...
full relative precision close to optimality, thank to the
structured ill-conditioning in the condensed matrix $K$. In other words,
we can refine the solution returned by the Cholesky solver accurately using
...
giving the following bounds for the errors in the inactive and actice components:
\begin{equation}
  \begin{aligned}
  ...
  \end{aligned}
\end{equation}
Hence, the error arises mostly in the active components $\widehat{d}_{z,\cactive}$.
We want to set $\varepsilon_K \ll \Xi$ to limit the loss of accuracy, but doing
so is getting not trivial as we are approaching the optimum. As example,
...
have only few inequality constraints.
