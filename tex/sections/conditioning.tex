...
as the implementation of these methods requires the use of
...
\begin{assumption}
  ...
  \item Second-order sufficiency: for every $h \in \nullspace\!\big(A(x^\star)\big)$,
...
\end{equation}
whre $m_i$ is the number of inequality constraints.
...
The duality measure is sometimes used to define the barrier parameter in IPM.
...
of the condensed matrix $K_\gamma$. All the results
apply directly to the matrix $K_\tau$, by setting the number
...
Witout regularization we have that $K_\gamma = W + H^\top D_s H + \gamma G^\top G$, with
the diagonal $D_s = S^{-1} V$.
We note by $m_a$ the cardinality of $\mathcal{B}$,
...
\end{equation}
We remind that $m_e$ is the total number of equality constraints,
and define $\ell := m_e + m_a$.

To establish the structured ill-conditioning of $K_\gamma$ by
...
\begin{corollary}
  ...
  \end{equation}
  For suitably chosen $Y$ and $Z$, % Sungho: maybe we should remind reader that $Y$ and $Z$ are the row and null spaces of constraint Jacobian.
  \begin{equation}
...
$\Delta D_\gamma = O(\epstol \overline{\sigma})$.
...
keeps the structured ill-conditioning highlighted in \eqref{eq:cond:svd}.
...
As $\eta_\ell = \Omega(\underline{\sigma})$, we deduce $\frac{\|K_\gamma \|}{\eta_\ell}
