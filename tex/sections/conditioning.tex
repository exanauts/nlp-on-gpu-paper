...
The condensed matrix $K$ appearing in \eqref{eq:kkt:condensed} is known to be increasingly ill-conditioned as the primal-dual iterate approaches to the local solution with active inequalities.
This behavior is amplified for the matrices $K_\gamma$ and $K_\tau$,
as the practical implementation of these methods requires the use of
large $\gamma$ and small $\tau$. In this section, we analyze the
...
The relative error after perturbing the right-hand side by $\Delta b$ can be expressed as follows:
\begin{subequations}
...
We now elaborate on what do we mean by structured ill-conditioning.
Here, we follow the analysis presented in \cite{wright1998ill}.
First, we show that the singular value decomposition of $K_\gamma$ can be structured as
\begin{equation}
  \label{eq:cond:svd}
  K_\gamma = \underbrace{\begin{bmatrix} U_L & U_S \end{bmatrix}}_{U}
...
\; ,
\end{equation}
where $U$ is an orthogonal matrix and $\Sigma$ is diagonal, which are subsequently partitioned into $U_L$, $U_S$, $\Sigma_L$, and $\Sigma_U$.Here, we assume that the two diagonal matrices $\Sigma_L$ and $\Sigma_S$ are both better conditioned than $K_\gamma$.
% The result is a consequence of the SVD decomposition,
% provided that the singular values satisfy
That is, $\frac{\sigma_1}{\sigma_{p}} \leq \frac{\sigma_1}{\sigma_n}$
...
In addition, we denote by $m_a$ the cardinality of $\mathcal{B}$,
and $p := m_a + m_e$. We denote by % Sungho: we already have used $p$ for primal variable. Should use a different symbol.
...
\begin{corollary}
  ...
  In addition, for suitably chosen $Y$ and $Z$, % Sungho: maybe we should remind reader that $Y$ and $Z$ are the row and null spaces of constraint Jacobian.
  \begin{equation}
...
\end{equation}
We deduce that the perturbation $\Delta K_\gamma$ is bounded by $O(\epstol \overline{\sigma})$. % Sungho: Here, we should elaborate more; I do not see this directly.
\begin{proposition}
