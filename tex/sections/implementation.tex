...
we use the ExaModels.jl modeling tool~\cite{shin2023accelerating} to \add{instantiate the evaluation of the nonlinear programs on the GPU}.
...
ExaModels.jl outperforms traditional modeling tools like JuMP and AMPL when even running on CPUs.
...
Within this paper, we always have excluded the compilation time from the reported timings.
}
...
\begin{itemize}
  ...
  \item \add{{\tt Panua Pardiso}: Implement shared-memory parallel supernodal Cholesky, \ldlt and \lblt factorizations on the CPU.}
