...
Furthermore, in Section \ref{sec:ipm:kkt}, we detail the Newton step
computation within each IPM iteration.
...
using a regular Newton method. For each primal-dual iterate
$w_k := (x_k, s_k, y_k, z_k, u_k, v_k)$ at iteration step $k$, the Newton step is applied as
$w_{k+1} = w_k + \alpha_k d_k$, where $d_k$ is a descent
direction computed by solving
\begin{equation}
...
In order to avoid overloading the notation, we omitted the subscript $k$ in the components of the KKT systems. However, it's important to remember that they are dependent on the iteration counter $k$.
...
Note that~\eqref{eq:kkt:unreduced} is not symmetric but can be symmetrized by performing simple block row and column operations.
...
in the condensed form~\eqref{eq:kkt:condensed}. To address the numerical error arising from such ill-conditioning, most of the
...
\eqref{eq:kkt:unreduced} is not symmetric but has a better
conditioning than the two alternatives \eqref{eq:kkt:augmented} and
\eqref{eq:kkt:condensed}.
...
Additionally, condensation may result in increased fill-in within the condensed system $K_1$~\cite[Section 19.3, p.571]{nocedal_numerical_2006}.
