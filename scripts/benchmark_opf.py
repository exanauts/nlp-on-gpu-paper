...
def import_benchmark(filename):
    df = pd.read_csv(filename, sep="\t", header=None, index_col=0, usecols=[0, 1, 2, 3, 4, 5])
    df.columns = ["it", "obj", "total", "AD", "lin"]
    df = df.astype({'it': int})
    return df
...
df_hckkt_cpu = import_benchmark(f"{RESULTS_DIR}/pglib-full-madnlp-hckkt-cholmod-8.csv")
